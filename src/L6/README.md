...
بنابراین، سیاست LaTeX این است که تغییرات اساسی در این کلاس‌ها انجام نشود—even اگر کاستی‌هایی در آن‌ها وجود داشته باشد.  
...
release-info شامل تاریخ انتشار و در صورت نیاز توضیح کوتاه یا شماره نسخه است.
...
draft → در پیش‌نویس داده می‌شود و سوئیچ `\if@myclass@draft` را تنظیم می‌کند.

name → فقط هنگام بارگذاری کلاس داده می‌شود و مقدارش در `\@myclass@name` ذخیره می‌شود.
